...
function addLink(e) {
    ...
    XHR.addEventListener('load', function(event) {
        ...
        if(responseObject.error) {
            ...
            newLinkContainer.setAttribute('id', 'link-div');
...
function editCollection(e, linkGroupArray) {
    ...
    for (var i=0; i < linkGroupArray.length; i++) {
        ...
        clearButtonText.className = 'error-text mui--text-center mui--text-body';

        var linkDivArray = linkGroupArray[i].getElementsByClassName('link-div');

        for (var j = 0; j < linkDivArray.length; j++) {
            var delLinkId = linkDivArray[j].firstElementChild.value;
            linkDivArray[j].appendChild(clearButton);
